...
- `src/deps.cljs` `:npm-deps` grows `@nats-io/jetstream` now and `@nats-io/kv` / `@nats-io/obj` in Phase 3 — each pinned in lockstep with the nats-core version. Root `package.json` (dev/test toolchain) grows the same dep so our Node and browser test legs resolve it.
