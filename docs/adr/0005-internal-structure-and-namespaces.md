...
The public surface is exactly **`nats-cljc.core`**, **`nats-cljc.codec`**, **`nats-cljc.jetstream`**, **`nats-cljc.kv`**, and **`nats-cljc.blocking.core`**, plus the opt-in codec namespaces **`nats-cljc.codec.<name>`** — those are public-by-require (ADR 0011 pins both the convention and the registry-miss hint that derives it), even though their contents are `^:no-doc`. **Every other namespace lives under an `impl` segment, nested per area**: root internals in `nats-cljc.impl.*` (`auth`, `error`, `protocol`, alongside the `jvm`/`js` legs this ADR already quarantined there), JetStream internals in `nats-cljc.jetstream.impl.*` (`acks`, `consumer`, `error`, `pub`, `pull`, `refill`, `stream`, alongside its `jvm`/`js` legs), KV internals in `nats-cljc.kv.impl.*` (`bucket`, `error`, alongside its `jvm`/`js` legs). Internal namespaces carry both signals: the `impl` path segment for editor autocomplete, `^:no-doc` for doc generators.
