...
- Owning a recommendation we don't ship means it must stay documented and version-tested: the README shows the promesa path alongside the native `await`/`deref` fallbacks, and the test aliases pin the promesa version the suite runs against. That documentation must carry one ClojureScript-specific caveat of the recommended layer (not of the library, whose return value is a native `js/Promise`): promesa's CLJS promise is its own type, so a rejection nobody observes is invisible to Node's `unhandledRejection` and the process exits `0` — silent failure where a bare native rejection would surface — so a top-level promesa chain must end in a `p/catch`.
