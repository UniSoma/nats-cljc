...
- title: '`(nats/subscribe conn subject handler)` returns a `Subscription` synchronously; the handler receives `{:subject :data}` with `:data` Transit-decoded'
  done: false
...
The thinnest end-to-end path through every layer: open a connection, publish a message, receive it on a subscription — identical `.cljc` code green on JVM (TCP), browser (WebSocket), and Node (WebSocket). `:transit` codec only.
...
ADRs: 0001 (non-blocking core; JVM=>TCP, CLJS=>WebSocket), 0002 (promesa one-shots + callback handler), 0004 (codec-centric, `:transit` default), 0005 (protocol + platform record behind a `.cljc` facade, `nats-cljc.*`), 0007 (handler delivery baseline).
